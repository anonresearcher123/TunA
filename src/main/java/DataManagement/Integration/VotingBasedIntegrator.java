...
//        System.out.println();
    ...
    private List<String> handleExternalData(int i, String relation){
        List<String> row = new LinkedList<>();
        List<String> extrDataList = solutionMap.get(i,relation);
        ...
        if(extrDataList != null && extrDataList.size() > 1) {
            ...
            handeled.add(extrDataList.get(0));
            row.add(extrDataList.get(0));
        } else {
            row.add(null);
        }
    ...
    private String vote(TunableQueryProcessor tqp, List<String> result){
        ...
        if(((MergeNode) node).getDissimilarity() <= threshold && confidence >= tqp.getSetting().get("Reliability")) {
            return determineIntegrationValue(result, node);
        } else {
